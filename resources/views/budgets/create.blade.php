...
<div class="container-fluid max-w-4xl">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div class="d-flex align-items-center">
            ...
            <h2 class="fw-bold mb-0 text-dark">{{ __('Nuevo Presupuesto de Obra') }}</h2>
        </div>
    </div>

    <div class="container mt-5">
        <div class="card shadow-sm p-4">
            <h2 class="mb-4 text-center">{{ __('Nuevo Presupuesto de Obra') }}</h2>
            ...
            <div class="col-lg-7 d-flex flex-column gap-4">
                ...
                <div class="card border-0 shadow-sm p-4">
                    <div class="d-flex align-items-center mb-3 pb-2 border-bottom">
                        ...
                        <h5 class="fw-bold mb-0 text-dark">{{ __('Mano de Obra (Peones)') }}</h5>
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label for="workers_quantity" class="form-label text-muted fw-semibold small mb-1">{{ __('Número de Trabajadores') }}</label>
                            <input type="number" class="form-control @error('workers_quantity') is-invalid @enderror" name="workers_quantity" id="workers_quantity" value="{{ old('workers_quantity') }}" placeholder="{{ __('Ej: 3') }}">
                            @error('workers_quantity') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-md-6">
                            <label for="price_x_hour" class="form-label text-muted fw-semibold small mb-1">{{ __('Precio Hora Peón') }}</label>
                ...
                <div class="card border-0 shadow-sm p-4">
                    <div class="d-flex align-items-center mb-3 pb-2 border-bottom">
                        ...
                        <h5 class="fw-bold mb-0 text-dark">{{ __('Personal Especializado') }}</h5>
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label for="staff_quantity" class="form-label text-muted fw-semibold small mb-1">{{ __('Nº de Capataces / Staff') }}</label>
                        ...
                        <div class="col-md-6">
                            <label for="staff_price" class="form-label text-muted fw-semibold small mb-1">{{ __('Precio Hora Capataz') }}</label>
                            <div class="input-group">
                                <span class="input-group-text bg-light"><i class="bi bi-currency-euro"></i></span>
                ...
                <div class="card border-0 shadow-sm p-4">
                    <div class="d-flex align-items-center mb-3 pb-2 border-bottom">
                        ...
                        <h5 class="fw-bold mb-0 text-dark">{{ __('Tiempo Estimado') }}</h5>
                    </div>
                    <div>
                        <label for="hours_quantity" class="form-label text-muted fw-semibold small mb-1">{{ __('Horas Totales Estimadas') }}</label>
                        <div class="input-group w-50">
                            <input type="number" class="form-control @error('hours_quantity') is-invalid @enderror" name="hours_quantity" id="hours_quantity" value="{{ old('hours_quantity') }}" placeholder="{{ __('Ej: 120') }}">
                            <span class="input-group-text bg-light">{{ __('horas') }}</span>
            ...
            <div class="col-lg-5">
                <div class="card border-0 shadow-sm p-4 h-100 bg-light bg-opacity-50 border border-primary border-opacity-10">
                    ...
                    <div class="d-flex align-items-center mb-3 pb-2 border-bottom border-secondary border-opacity-25">
                        ...
                        <h5 class="fw-bold mb-0 text-dark">{{ __('Gestión de Materiales') }}</h5>
                    </div>

                    <div class="mb-4">
                        <p class="text-muted small fw-semibold text-uppercase mb-2">{{ __('Añadir del inventario') }}</p>

                        <div class="mb-3">
                            <label for="existing_material_id" class="form-label text-muted small mb-1">{{ __('Selecciona el Material') }}</label>
                            <select class="form-select border-secondary border-opacity-25 shadow-sm" name="existing_material_id" id="existing_material_id">
                                <option value="">{{ __('-- No añadir ninguno por ahora --') }}</option>
                        ...
                        <div>
                            <label for="existing_material_quantity" class="form-label text-muted small mb-1">{{ __('Cantidad a usar en la obra') }}</label>
                    ...
                    <div class="form-check form-switch p-3 bg-white border rounded shadow-sm mb-3 d-flex align-items-center gap-2 cursor-pointer" onclick="document.getElementById('toggleMaterial').click();">
                        ...
                        <label class="form-check-label fw-bold text-dark cursor-pointer ms-2" for="toggleMaterial" style="user-select: none;">
                            {{ __('Añadir material nuevo al catálogo') }}
                        </label>
                    </div>
                    ...
                    <div id="seccionMaterial" class="bg-white p-3 border rounded shadow-sm" style="display: {{ old('crear_material_nuevo') ? 'block' : 'none' }}; border-left: 4px solid var(--color-verde-oscuro) !important;">
                        <h6 class="fw-bold text-verde-oscuro mb-3"><i class="bi bi-plus-circle me-1"></i> {{ __('Datos del Nuevo Material') }}</h6>

                        <div class="mb-2">
                            <label class="form-label text-muted small mb-1">{{ __('Nombre del Material') }}</label>
                        ...
                        <div class="mb-2">
                            <label class="form-label text-muted small mb-1">{{ __('Proveedor (Opcional)') }}</label>
                        ...
                        <div class="row g-2 mb-2">
                            <div class="col-6">
                                <label class="form-label text-muted small mb-1">{{ __('Precio Unid. (€)') }}</label>
                            ...
                            <div class="col-6">
                                <label class="form-label text-muted small mb-1">{{ __('Stock Comprado') }}</label>
                        ...
                        <div class="p-2 bg-light border rounded mt-3">
                            <label class="form-label text-primary fw-semibold small mb-1">{{ __('Cant. a usar en esta obra') }}</label>
        ...
        <div class="card border-0 shadow-sm p-3 mt-4">
            <div class="d-flex justify-content-end gap-3">
                <a href="{{ route('budgets.index') }}" class="btn btn-light border px-4 py-2 fw-medium">{{ __('Cancelar') }}</a>
                <button type="submit" class="btn btn-verde-oscuro fw-semibold px-4 py-2 shadow-sm">
                    <i class="bi bi-save me-2"></i> {{ __('Generar Presupuesto') }}
                </button>
            </div>
        </div>
