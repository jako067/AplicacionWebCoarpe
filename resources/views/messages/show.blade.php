...
@section('title', __('Detalle del Mensaje'))
...
<div class="container-fluid max-w-4xl">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div class="d-flex align-items-center">
            <a href="{{ route('messages.index') }}" class="btn btn-sm btn-outline-secondary me-3 shadow-sm" title="{{ __('Volver') }}">
                <i class="bi bi-arrow-left"></i> {{ __('Volver') }}
            </a>
            <h2 class="fw-bold mb-0 text-dark">{{ __('Detalle del Mensaje') }} #{{ $message->id }}</h2>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm overflow-hidden mb-4">
            ...
            <div class="row align-items-center g-3">
                <div class="col-md-8">
                    <span class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 mb-2">
                        {{ __('Asunto del Trámite') }}
                    </span>
                    <h4 class="fw-bold text-dark mb-0">{{ $message->subject }}</h4>
                </div>
                <div class="col-md-4 text-md-end text-muted small">
                    <div class="mb-1">
                        <i class="bi bi-person me-1"></i> <strong>{{ __('Autor:') }}</strong> {{ $message->user->name ?? __('Sin usuario') }}
                    </div>
                    <div>
                        <i class="bi bi-calendar-event me-1"></i> <strong>{{ __('Enviado:') }}</strong> {{ $message->created_at }}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card-body p-4 bg-light bg-opacity-25">
            <p class="text-muted fw-semibold small text-uppercase mb-2">{{ __('Contenido del Mensaje') }}</p>
        ...
        <div class="card-footer bg-white p-4 border-top">
            <p class="text-muted fw-semibold small text-uppercase mb-2">{{ __('Documentación Adjunta') }}</p>
            ...
            @if ($message->document)
                <div class="d-flex flex-column flex-sm-row align-items-sm-center justify-content-between p-3 bg-light rounded-3 border gap-3">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <h6 class="fw-bold text-dark mb-0">{{ __('Archivo Adjunto') }}</h6>
                            <small class="text-muted">{{ __('El mensaje incluye un documento justificante.') }}</small>
                        </div>
                    </div>
                    ...
                    <a href="{{ asset('storage/' . $message->document) }}" target="_blank" class="btn btn-verde-oscuro fw-semibold shadow-sm px-4 py-2 flex-shrink-0">
                        <i class="bi bi-box-arrow-up-right me-2"></i> {{ __('Ver documento') }}
                    </a>
                </div>
            @else
                <div class="p-3 bg-light rounded-3 text-center text-muted small border border-dashed">
                    <i class="bi bi-paperclip me-1"></i> {{ __('Este mensaje no contiene ningún archivo o documento adjunto.') }}
                </div>
            @endif
        </div>
