...
<div class="container-fluid max-w-4xl">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div class="d-flex align-items-center">
            ...
            <h2 class="fw-bold mb-0 text-dark">{{ __('Editar Daily Work') }}</h2>
        </div>
    </div>

    <div class="card border-0 shadow-sm p-4">
        <h3 class="mb-4 text-center fs-5 text-muted text-uppercase tracking-wider">{{ __('Modificar Registro') }} #{{ $dailyWork->id }}</h3>
        ...
        <form action="{{ route('daily_work.update', $dailyWork->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea class="form-control @error('reporte') is-invalid @enderror" name="reporte" id="reporte" rows="5" placeholder="{{ __('Describe las tareas ejecutadas en la jornada...') }}">{{ old('reporte', $dailyWork->reporte) }}</textarea>
                @error('reporte')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6 mb-4">
                    ...
                    <div class="input-group">
                        ...
                        <textarea class="form-control @error('incidences') is-invalid @enderror" name="incidences" id="incidences" rows="1" placeholder="Ej: Rotura de maquinaria, falta de suministro">{{ old('incidences', $dailyWork->incidences) }}</textarea>
                        @error('incidences')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="d-flex justify-content-end gap-3">
                ...
                <button type="submit" class="btn btn-verde-oscuro fw-semibold px-4 py-2 shadow-sm">
                    <i class="bi bi-check-circle me-2"></i> {{ __('Actualizar') }}
                </button>
            </div>
