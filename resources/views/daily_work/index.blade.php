...
<div class="container-fluid max-w-4xl">
    ...
    <div class="card border-0 shadow-sm p-4 mb-4">
        <h2 class="fw-bold mb-1 text-dark">Partes de Trabajo Diario</h2>
        <p class="text-muted mb-4">Historial de producción diaria, evaluaciones de rendimiento y gestión de incidencias.</p>
        ...
        <form method="GET" action="{{ route('daily_work.index') }}" class="mb-4 bg-light p-3 border rounded-3 shadow-sm">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-4">
                    <label class="form-label text-muted fw-semibold small mb-1">Filtrar por Cuadrilla</label>
                    <select name="group_id" class="form-select border-secondary border-opacity-25 shadow-sm">
                        <option value="">Todos los grupos / cuadrillas</option>
                ...
                <div class="col-md-4">
                    <label class="form-label text-muted fw-semibold small mb-1">Filtrar por Fecha</label>
        ...
        <div class="border rounded-3 overflow-hidden shadow-sm bg-white">
            <div class="bg-light border-bottom p-3 d-flex justify-content-between align-items-center">
                <span class="text-dark fw-medium"><i class="bi bi-journal-text me-2 text-verde-oscuro"></i> Registros de Producción</span>
                <span class="badge bg-secondary rounded-pill">{{ $dailyWorks->count() }} visibles</span>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light">
                        <tr>
                            <th class="text-muted small fw-semibold ps-3">Grupo / Cuadrilla</th>
                            <th class="text-muted small fw-semibold">Reporte Diario</th>
                            <th class="text-muted small fw-semibold">Fecha</th>
                            <th class="text-muted small fw-semibold text-center">Evaluación</th>
                            <th class="text-muted small fw-semibold">Incidencias</th>
                        ...
                        @forelse($dailyWorks as $dailyWork)
                            <tr onclick="window.location='{{ route('daily_work.show', $dailyWork->id) }}'" style="cursor:pointer;" class="transition-all hover-bg-light">
                                <td class="ps-3 fw-bold text-dark">
                                    <i class="bi bi-building me-1 text-secondary"></i> {{ $dailyWork->group->name ?? 'Sin grupo' }}
                                </td>
                                <td class="text-muted">
                                ...
                                <td class="text-danger small">
                                    {{ \Illuminate\Support\Str::limit($dailyWork->incidences, 40) ?: 'Ninguna' }}
                                </td>
                                <td class="text-end pe-3" onclick="event.stopPropagation();">
                                    <div class="d-flex gap-2 justify-content-end">
                                        <a href="{{ route('daily_work.edit', $dailyWork->id) }}" class="btn btn-sm btn-outline-primary" title="Editar">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <form action="{{ route('daily_work.destroy', $dailyWork->id) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Seguro que quieres eliminar este registro de producción?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-5 text-muted">
                                    ...
                                    No se encontraron registros de trabajo para este criterio.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
<script>
    ...
    window.addEventListener('scroll', async () => {
        ...
        // Si el usuario está cerca del final de la página (a 200px o menos)
        if (window.innerHeight + window.scrollY >= document.body.offsetHeight - 200) {
            ...
            // Si el servidor ya no tiene más páginas de datos, nos detenemos
            if (data.data.length === 0) return;

            const table = document.getElementById('daily-work-table');

            data.data.forEach(item => {
                const groupName = item.group ? item.group.name : 'Sin grupo';
                const shortReporte = item.reporte ? (item.reporte.length > 40 ? item.reporte.substring(0, 40) + '...' : item.reporte) : '';
                const shortIncidences = item.incidences ? (item.incidences.length > 40 ? item.incidences.substring(0, 40) + '...' : item.incidences) : 'Ninguna';
                const evaluation = item.evaluation ? item.evaluation : '-';

                // Inyección dinámica manteniendo la coherencia visual al 100%
                table.innerHTML += `
<tr onclick="window.location='/daily_work/${item.id}'" style="cursor:pointer;" class="transition-all hover-bg-light">
    ...
    <td class="text-end pe-3" onclick="event.stopPropagation();">
        <div class="d-flex gap-2 justify-content-end">
            <a href="/daily_work/${item.id}/edit" class="btn btn-sm btn-outline-primary" title="Editar">
                <i class="bi bi-pencil"></i>
            </a>
            <form method="POST" action="/daily_work/${item.id}" class="d-inline" onsubmit="return confirm('¿Seguro que quieres eliminar este registro de producción?')">
