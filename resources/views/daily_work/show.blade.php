    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div class="d-flex align-items-center">
            ...
            <h2 class="fw-bold mb-0 text-dark">
                {{ __('Detalle del Daily Work') }} <span class="text-verde-oscuro">#{{ $dailyWork->id }}</span>
            </h2>
        </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-4">
            <div class="bg-white p-3 border rounded-3 shadow-sm h-100">
                <p class="text-muted fw-semibold small mb-1 text-uppercase tracking-wider" style="font-size: 0.75rem;">{{ __('Grupo') }}</p>
                <h5 class="fw-bold text-dark mb-0">
                    <i class="bi bi-building text-secondary me-1"></i> {{ $dailyWork->group->name ?? __('Sin grupo') }}
                </h5>
            </div>
        </div>
        <div class="col-md-4">
            <div class="bg-white p-3 border rounded-3 shadow-sm h-100">
                <p class="text-muted fw-semibold small mb-1 text-uppercase tracking-wider" style="font-size: 0.75rem;">{{ __('Fecha') }}</p>
                <h5 class="fw-bold text-dark mb-0">
        ...
        <div class="col-md-4">
            <div class="bg-white p-3 border rounded-3 shadow-sm h-100">
                <p class="text-muted fw-semibold small mb-1 text-uppercase tracking-wider" style="font-size: 0.75rem;">{{ __('Evaluación') }}</p>
                <h5 class="fw-bold mb-0">
                    <span class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-10 px-2 py-1 fs-6 fw-bold">
                        {{ $dailyWork->evaluation ?? __('Sin evaluar') }}
                    </span>
                </h5>
            </div>
        </div>
    ...
    <div class="card border-0 shadow-sm p-4">
        ...
        <div class="mb-4">
            <h5 class="fw-bold text-dark border-bottom pb-2 mb-3">
                <i class="bi bi-journal-text me-2 text-verde-oscuro"></i> {{ __('Reporte') }}
            </h5>
        ...
        <div>
            <h5 class="fw-bold text-dark border-bottom pb-2 mb-3">
                <i class="bi bi-exclamation-triangle me-2 text-danger"></i> {{ __('Incidencias') }}
            </h5>
            ...
            @else
                <div class="bg-light bg-opacity-50 p-3 rounded-3 border text-muted small">
                    <i class="bi bi-check-circle-fill text-success me-2 fs-6"></i> {{ __('Jornada completada sin registrar contratiempos operativos.') }}
                </div>
            @endif
        </div>
